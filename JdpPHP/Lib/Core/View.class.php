...
class View {
     ...
     * 初始化设置
     */
    public function __construct($tplDir = '', $parseDir = '') {
        $this->tplDir = empty($tplDir)?APP_TPL_PATH:$tplDir;
        $this->parseDir = empty($parseDir)?APP_TPL_PARSE_PATH:$parseDir;
        $this->cacheDir = empty($cacheDir)?APP_HTML_CACHE_PATH:'';
    ...
    public function display($file = '') {
        ...
        if ('' == $file) {
            $this->tpl_file = $this->tplDir . '/' .$default_tpl_name . $this->tplExt;
        } else {
            $this->tpl_file = $file;
        } 
        ...
        $this->parse_file = $this->parseDir . '/' . md5($this->tpl_file) . '.php'; //设置编译文件路径       
        $this->_showContent();
    } 
    ...
    public function compile() {
        ...
        if (!file_exists($this->parse_file) || $debug == 1) {
            ...
            $isChanged = $this->is_changed($this->tpl_file);
    ...
    public function is_changed($f){
        ...
        if(filemtime($f)+5>=filemtime($this->parse_file)){
           $changed = true; 
           return $changed;
        }
        ...
        foreach($arr[2] as $k=>$v){
            $file = $this->tplDir . "/" . $v. $this->tplExt;
            
            if(filemtime($file)>=filemtime($this->parse_file)){
                $changed = true;
                break;
            }
            $this->is_changed($file);
    ...
    function parse() {
        ...
        $this->tpl_con = preg_replace_callback($mode,  array($this, 'include_file'), $this->tpl_con); 
    ...
    private function _showContent() {
        ...
        if ($this->cache_on && isset($cache_rules[$rules_key])) {
            ...
            $this->cache_file = $this->cacheDir . $rule_str . $this->tplExt; //设置缓存文件路径
     ...
     * 读取包含文件的内容
     */
    private function include_file($match) {
        $file = $this->tplDir . "/" . $match[1] . $this->tplExt;
            //dump($file);
        if (file_exists($file)) {
            $include_con = file_get_contents($file); //读取模板文件内容

            $mode = '/'.$this->left.'\s*include\s+file\s*=\s*\'(.+)\''.'\s*\/\s*'.$this->right.'/iU';
            $include_con = preg_replace_callback($mode, array($this, 'include_file'), $include_con);
